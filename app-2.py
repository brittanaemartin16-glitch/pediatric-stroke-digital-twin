...
st.markdown('<div class="section-label">Whole-project pathway</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="flow">'
...
with middle:
    ...
    st.image(
        CT_IMAGE,
        caption="Highlighted cortical region · Not a real scan",
        use_container_width=True,
    )
...
exp_col, explanation_col = st.columns([1.25, .75], gap="large")
with exp_col:
...
with explanation_col:
    ...
    st.markdown(
        f'<div class="interpret"><b>{top_factor}</b> currently has the largest effect. '
        'Longer bars mean greater influence on this demonstration—not proof of cause.</div>',
        unsafe_allow_html=True
    )
