...
st.markdown(
    '<div class="notice"><b>Research demonstration:</b> All patient details and estimates are '
    'fictional and are presented for educational purposes only.</div>',
    unsafe_allow_html=True,
)

st.markdown('<div class="section-label">How the Research Stages Work Together</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="flow">'
    '<div class="flow-card"><span class="icon">🧪</span><b>Theme A · Treatment Discovery</b>'
    '<small>Identifies molecular targets and evaluates hydrogel-based delivery.</small></div>'
    '<div class="arrow">→</div>'
    '<div class="flow-card"><span class="icon">🧠</span><b>Theme B · Personalized Prediction</b>'
    '<small>Transforms patient data into updated epilepsy-risk and treatment-response estimates.</small></div>'
    '<div class="arrow">→</div>'
    '<div class="flow-card"><span class="icon">🏥</span><b>Theme C · Access to Care</b>'
    '<small>Evaluates barriers that may delay testing, treatment, and follow-up.</small></div>'
...
factor_chart = (
    alt.Chart(factor_df)
    .mark_bar(cornerRadiusEnd=7, color="#247ba0", size=34)
    .encode(
        x=alt.X(
            "Relative contribution:Q",
            title="Relative contribution to the current estimate",
            axis=alt.Axis(labelFontSize=15, titleFontSize=17, titlePadding=16),
        ),
        y=alt.Y(
            "Factor:N",
            sort="-x",
            title=None,
            axis=alt.Axis(labelFontSize=17, labelLimit=600, labelPadding=14),
        ),
